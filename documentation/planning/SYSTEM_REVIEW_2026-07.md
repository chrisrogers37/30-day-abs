...
Each entry below is written to be copy-pasteable into a GitHub issue. Status values: `OPEN`, `IN PROGRESS`, `DONE`, `WONTFIX`.
...
### P0-1: Design batch scoring uses mismatched answer keys — Q4–Q6 always marked wrong — `OPEN`
...
### P0-2: Design scoring results are computed but never displayed — `OPEN`
...
### P0-3: ID-based `rollout_decision` correct answer is always "No" — `OPEN`
...
### P0-4: "Generate New Scenario" does not fully reset session state — `OPEN`
...
### P0-5: Analysis grading hardcodes `alpha=0.05`, ignoring the scenario's alpha — `OPEN`
...
### P0-6: UI validation hardcodes 50/50 allocation regardless of scenario — `OPEN`
...
### P0-7: Duration answers use two different formulas that disagree for unequal allocation — `OPEN`
...
### P0-8: `validate_test_duration` / `suggest_parameter_adjustments` crash on call — `OPEN`
...
### P0-9: `score_analysis_answers()` raises `KeyError` when `business_target_absolute` is None — `OPEN`
...
### P1-1: `get_z_score()` lookup table returns wrong critical value for two-tailed α=0.10 — `OPEN`
...
### P1-2: Inconsistent `z_beta` tail direction across sample-size helpers — `OPEN`
...
### P1-3: `relative_lift_to_absolute()` percent/decimal convention conflicts with the rest of the codebase — `OPEN`
...
### P1-4: Revenue "confidence interval" is not a confidence interval — `OPEN`
...
### P1-5: Simulation volume is hardcoded to 30 days, decoupled from the designed sample size — `OPEN`
...
### P1-6: Generator path sends the full scenario prompt twice per API call — `OPEN`
...
### P1-7: Generator accepts below-threshold quality scenarios on the final attempt — `OPEN`
...
### P1-8: `LLMOutputParser.parsing_errors` accumulates across calls — `OPEN`
...
### P1-9: Bounds disagree three ways: prompt vs Pydantic schema vs guardrails — `OPEN`
...
### P1-10: Guardrails hard-fail on MDE/lift rounding — `OPEN`
...
### P1-11: Fallback scenarios masquerade as success — `OPEN`
...
### P1-12: Client retry loop treats `AuthenticationError` like a transient failure; typed exceptions unused — `OPEN`
...
### P1-13: No JSON mode; expensive defaults — `OPEN`
...
### P1-14: CSV export writes the wrong "correct answer" column — `OPEN`
...
### P1-15: Legacy analysis answer key has 6 questions; the live quiz has 7 — `OPEN`
...
### P2-1: `ui/streamlit_app.py` is a 1,813-line monolith — `OPEN`
...
### P2-2: Two parallel quiz/scoring systems that disagree — `OPEN`
...
### P2-3: UI bypasses the LLM orchestrator, losing retries/fallback/novelty — `OPEN`
...
### P2-4: Dead and unused code across all layers — `OPEN`
**Inventory:**
...
### P2-5: `core/logging.py` gives the "pure" core filesystem side effects — `OPEN`
...
### P2-6: Duplicated statistical helpers — `OPEN`
**Area:** `core/utils.py`
...
### P2-7: Schema ↔ core bounds and enums don't round-trip — `OPEN`
...
### P2-8: Module docstrings and READMEs describe an aspirational system — `OPEN`
...
### INFRA-1: No CI test workflow — `OPEN`
...
### INFRA-2: Dependency gaps and unpinned requirements — `OPEN`
...
### INFRA-3: UI has zero real tests; some "tests" can't fail — `OPEN`
...
### INFRA-4: Real `llm.client` retry/error paths untested — `OPEN`
...
### INFRA-5: Root-level `test_scenario_variety.py` is a manual API script, not a test — `OPEN`
...
### INFRA-6: Integration tests don't test what their names claim — `OPEN`
...
### INFRA-7: Stale/inflated documentation claims — `OPEN`
...
These carry over from the prior statistical audit and remain valid:
